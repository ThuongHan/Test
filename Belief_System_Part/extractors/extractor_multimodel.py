...
Theoretical basis
-----------------
This extractor implements a Multi-Model Agreement approach for building an
organisational belief system.

It is inspired by Hashemi et al. (2024)'s LLM-RUBRIC principle that text
assessment becomes more reliable when the text is evaluated across multiple
rubric dimensions rather than through one direct judgment.

Important distinction:
- The original LLM-RUBRIC framework uses human-labelled data and a calibration
  network to align LLM evaluations with human judges.
...
- Instead, it approximates robustness through three independent LLM passes with
  different extraction framings and retains only beliefs that receive semantic
...
Pipeline
--------
  Pass 1 → Conservative explicit belief extraction
  Pass 2 → Strongly grounded implicit belief extraction
  Pass 3 → Strategic assumption and normative claim extraction
...
Design choices
--------------
- Same base model with different temperatures and system-prompt framings is used
  as a pragmatic stand-in for true multi-model diversity.
...
- Agreement is counted by DISTINCT PASSES, not by the number of candidate
  ...
  duplicate beliefs.
- MIN_AGREEMENT = 2 out of 3 passes gives a majority-vote filter.
...
# ── SYSTEM PROMPTS ────────────────────────────────────────────────────────────
# These are deliberately NOT sensemaking prompts.
# They are belief-system extraction prompts using different analytical framings.
...
def _quote_is_supported(source_excerpt: str, text: str) -> bool:
    ...
    This avoids retaining hallucinated quotes.
    """
...
def _call_model(
    ...
    """
    Call the OpenAI API with a given system prompt and temperature.
...
def _same_belief_candidate(a: dict, b: dict) -> bool:
    ...
    We primarily compare belief_statement similarity.
    Category mismatch is allowed only when semantic similarity is high, because
    models may classify the same belief differently.
    """
    ...
    # Higher bar if categories differ.
    if sim >= 0.55:
        return True
...
def _aggregate_confidence(records: list[dict], pass_count: int) -> str:
    ...
    Agreement is treated as additional evidence.
    """
    scores = [
    ...
    if pass_count == N_MODELS and avg >= 2.5:
        return "high"
...
def _select_representative(records: list[dict]) -> dict:
    ...
    3. Longer belief statement
    4. Shorter source excerpt
    """
    return max(
        records,
        key=lambda r: (
...
def _cluster_and_filter(all_beliefs: list[dict]) -> list[dict]:
    """
    Group semantically equivalent beliefs across passes and retain only beliefs
    meeting the MIN_AGREEMENT threshold.

    Critical point:
    agreement_count is based on DISTINCT PASSES, not number of beliefs in the
    cluster.
    """
    ...
    for cluster in clusters:
        ...
        retained_record = {
            ...
            "agreement_score": round(pass_count / N_MODELS, 3),
        }
...
def _final_deduplicate(beliefs: list[dict]) -> list[dict]:
    ...
    - beliefs with very similar statements and the same category are merged;
    - the higher-agreement / higher-confidence record is retained.
    """
    final: list[dict] = []
...
def _normalise_seed_belief(seed: dict[str, Any]) -> dict[str, Any] | None:
    """
    Normalize optional seed beliefs into the current schema.
    ...
    return {
        ...
        "agreement_score": float(seed.get("agreement_score", 1 / N_MODELS)),
...
class MultiModelExtractor(BaseExtractor):
    """
    Method 3 — Multi-Model Agreement Extraction.

    This method builds a belief system through cross-pass agreement rather than
    through sensemaking theory.

    It runs three independent LLM passes, clusters semantically equivalent
    beliefs, and retains only beliefs surfaced by at least MIN_AGREEMENT distinct
    passes.
    """
    ...
    def extract(self, text: str, source_label: str) -> list[dict]:
        """
        Run all three model passes on a single text segment, cluster results,
    ...
    def run_pipeline(
        ...
        """
        Full Step 2 pipeline using multi-model agreement extraction.
        ...
        print(f"\n[MultiModelExtractor] Documents prepared : {len(corpus)}")
        print(f"[MultiModelExtractor] Model              : {BASE_MODEL}")
        print(f"[MultiModelExtractor] Agreement threshold: {MIN_AGREEMENT}/{N_MODELS} passes")
        print(f"[MultiModelExtractor] Similarity threshold: {SIMILARITY_THRESHOLD}")
        ...
        for i, doc in enumerate(docs_to_process, start=1):
            ...
            n_full = sum(
                1 for b in beliefs
                if b.get("agreement_count", 0) == N_MODELS
            )

            print(
                f"  → {n_total} beliefs retained | "
                f"{n_full} with full agreement ({N_MODELS}/{N_MODELS})"
            )
